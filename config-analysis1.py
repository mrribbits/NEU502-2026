...
# No metadata filtering needed for this experiment
...
_spatial_filter = "HFC"  # "maxwell" | "HFC" | "ZCA" | "none"
...
# assert that only one of these is true
assert(sum([use_maxwell_filter, _do_HFC, _do_ZCA]) <= 1)
print('    - using spatial filter method:', _spatial_filter)


# ZCA settings
_zca_ext_order = 3       # External SSS order (default: 3)
_zca_threshold = 0.99    # GED eigenvalue threshold (default: 0.99)

# HFC settings
_hfc_order = 3

# Maxwell filter settings
mf_int_order = 8
mf_ext_order = 3


_manual_channels = True # manually check bad channels
_auto_ica = False # automatically check ica components
_manual_ica = False # manually check ica components
_skip_on_deriv = False # skip if derivatives folder exist


# reference regression
_regress_ref = True
_regress_ref_timevarying = False
_regress_ref_method = "window"
_regress_ref_window = 100.0
_regress_ref_freqs = [(None, 5.0)]
...
# eog_channels = ['xpos_left', 'ypos_left']
...
# Resampling: 1200 Hz / 3 = 400 Hz final rate (sufficient for 30 Hz lowpass)
# For 150Hz content, Nyquist requires at least 300Hz sampling rate, so epochs_decim = 2, gives me  600Hz. 
# That's well above Nyquist for 150Hz.
epochs_decim = 2
...
# %% BEAMFORMER SETTINGS (LCMV) --------------------------------------------

"""
BEAMFORMER CONFIGURATION NOTES:

The LCMV beamformer is a spatial filtering technique that reconstructs
source activity by computing an optimal set of weights for each source
location. It is particularly well-suited for OPM-MEG data.

Key parameter choices:

1. pick_ori='max-power' + weight_norm='unit-noise-gain':
   - Standard scalar beamformer with depth bias correction
   - Recommended for most analyses
   
2. pick_ori='max-power' + weight_norm='nai':
   - Neural Activity Index
   - Good for comparing power between conditions
   
3. pick_ori='vector' + weight_norm='unit-noise-gain-invariant':
   - Vector beamformer with all three dipole components
   - Useful when orientation is important

4. Depth bias handling:
   - Use weight_norm for normalization approach (preferred)
   - Use depth parameter for forward model weighting
   - When computing contrasts, depth bias cancels out

For more details, see:
- MNE tutorial: https://mne.tools/stable/auto_examples/inverse/50_beamformer_lcmv.html
"""

# Enable/disable beamformer analysis
_run_beamformer = True

# Beamformer regularization parameter
# Controls the amount of regularization applied to the covariance matrix
# Typical values: 0.01 to 0.10
# Higher values = more regularization = more stable but potentially less accurate
_beamformer_reg = 0.01

# Orientation selection for dipole sources
# 'max-power': Optimize dipole orientation for maximum power (scalar output)
# 'vector': Return all three dipole components (vector output)
# None: Use fixed orientation from forward model
_beamformer_pick_ori = 'max-power'

# Weight normalization method
# 'unit-noise-gain': Depth bias correction via noise normalization
# 'nai': Neural Activity Index, normalizes by noise (good for power comparisons)
# None: No normalization (biased toward superficial sources)
# Note: Use 'unit-noise-gain-invariant' for vector beamformers
_beamformer_weight_norm = 'nai'

# Depth weighting parameter
# Controls compensation for depth bias in forward model
# 0.0: No depth weighting
# 0.8: Standard depth weighting (recommended for comparing single conditions)
# None: No depth weighting (recommended when using weight_norm or comparing contrasts)
_beamformer_depth = 0.8

# Rank parameter for covariance matrices
# 'info': Use rank from data info (recommended)
# dict: Specify rank per channel type, e.g., {'mag': 64}
# None: Auto-detect rank
_beamformer_rank = 'info'

# Output type for beamformer
# 'time': Time-locked analysis (apply to evoked) - PRIMARY
# 'power': Power analysis (apply to covariance) - SECONDARY
# 'both': Run both analyses (RECOMMENDED)
_beamformer_output_type = 'time'

# Time window for power beamformer covariance estimation
# Time in seconds relative to epoch onset
_beamformer_power_tmin = 0.0
_beamformer_power_tmax = epochs_tmax

# Save beamformer filters for later reuse
_beamformer_save_filters = True

# Add beamformer results to HTML report
_beamformer_add_to_report = True

# Number of time points to show in report for STCs
_beamformer_report_n_time_points = 51
